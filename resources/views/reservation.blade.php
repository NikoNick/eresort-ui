...
<script type="text/javascript">
	...
	function loadExtraItem(extra_item) {
		...
		$.each(extra_item, function(index, item) {
			...
			// var $extra_item = 
			// '<div id="' + id_item + '" class="tambahan animation anim-slide-down-up disappear">' +
			// 	'<span>' +
			// 		'<input type="checkbox" id="checkbox-' + id_item + '" class="checkbox fillable">' +
			// 		'<label for="checkbox-' + id_item + '"></label>' +
			// 	'</span>' +
			// 	'<div class="nama">' +
			// 		'<b>' + nama_item + '</b>' +
			// 	'</div>' +
			// 	'<div class="harga"><p>' + string_harga + '/pcs</p><input id="' + id_harga + '" type="hidden" name="harga" value="' + harga + '"></div>' +
			// '</div>';
		...
		$('.tambahan input.checkbox').on('change', checkboxListener2);
		$('.order input[type="number"]').on('keyup', inputNumberListener);
		$('.order input[type="number"]').on('change', inputNumberListener);
		$('.order input.checkbox').on('change', checkboxListener);
